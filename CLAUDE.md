...
**Benchmark de competencia** (sep-2026) en `docs/competencia/` (con `gap_analysis.md`, `roadmap_producto.md` y `spec_entrevista_ia.md`):
...
| PBN | `.lector-notam .lector-pbn` | `#5C4520` bronce de instrumento (tokens `--av-pbn-*`) |
...
- **Nada nace abierto.** Una tabla, secuencia o función nueva no tiene permisos para `anon`
  ni `authenticated` (migración `20260911173659`). La migración que la crea hace
...
- **El archivo lleva la versión que registró la base.** Después de aplicar:
  ...
  archivo se llama `<version>_<nombre>.sql`. El repo tiene un archivo por fila del historial
