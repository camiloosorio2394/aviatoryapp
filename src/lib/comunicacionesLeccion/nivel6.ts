...
export const NIVEL_6: DocScreen[] = [
  // ── 41 ──────────────────────────────────────────────────────────────────
  {
    ...
    blocks: [
      {
        kind: "p",
        text: "Capítulos 41 a 50. Lo que no va por voz o no va por VHF: ATIS y VOLMET, CPDLC y la autorización de salida por data link, ACARS, ADS-C, HF y SELCAL, y cómo se combinan en la operación oceánica.",
      },
      verificar(
        "Este nivel se apoya en documentos que no están cargados. Los mensajes CPDLC dependen del Doc 4444 cap. 14 y del Doc 10037 (GOLD); la fraseología oceánica, de SELCAL y de meteorología, del Doc 9432 cap. 8 a 11, el Anexo 10 Vol. II y el NAT Doc 007. Todo lo marcado (VERIFICAR) se confirma ahí antes de tomarlo como norma. Los formatos oceánicos cambian por región: se estudian en la documentación de cada una (NAT Doc 007, AIP de cada Estado, Doc 7030).",
      ),
      { kind: "sub", text: "¿Qué es?" },
      {
        kind: "p",
        text: "El **servicio automático de información terminal (ATIS)** es el «suministro automático de información regular, actualizada, a las aeronaves que llegan y a las que salen, durante las 24 horas o determinada parte de las mismas» (Doc 4444, cap. 1; Doc 9432, cap. 1).",
      },
      { kind: "p", text: "Tiene dos formas, definidas en los mismos documentos:" },
      {
        kind: "glosario",
        items: [
          { k: "ATIS-voz", v: "radiodifusión vocal continua y repetitiva en una frecuencia." },
          { k: "ATIS-D (D-ATIS)", v: "el mismo ATIS entregado por enlace de datos, como texto en la cabina." },
        ],
      },
      {
        kind: "p",
        text: "Cada emisión se identifica con una letra del alfabeto fonético («information Bravo», «information Charlie»). Cuando cambia algo importante, cambia la letra.",
      },
      ...
      COMO_LEER,
      verificar(
        "El contenido obligatorio del ATIS y la regla de cambio de letra (orden alfabético consecutivo) no están en las fuentes cargadas: consultar el Anexo 11, cap. 4. No hay ejemplo de cómo anuncia el ATC una nueva letra vigente porque esa fraseología está en el Doc 4444 cap. 12, no cargado. La redacción de 4.5.7.5.1 c) se confirma en el Doc 4444 16.ª ed. (4.5.7.5). Unidad del QNH y práctica local de acuse del ATIS en Colombia: AIP Colombia (GEN y AD 2) y RAC.",
      ),
      ...ejemplo(
        "Ejemplo 1 · Salida: solicitud de puesta en marcha con acuse del ATIS (Doc 9432, 4.2.2)",
        [
      ...
      ...ejemplo(
        "Ejemplo 2 · Rodaje con acuse del ATIS (Doc 9432, 4.4.3)",
        [
      ...
      ...ejemplo(
        "Ejemplo 3 · Llegada: llamada inicial a aproximación (Doc 9432, 7.3.1)",
        [
      ...
      ...ejemplo(
        "Ejemplo 4 · Sin ATIS disponible (Doc 9432, 4.2.1)",
        [
      ...
      ...ejemplo(
        "Ejemplo 5 · Instrucción de escuchar el ATIS (Doc 9432, 2.8.2.2)",
        [
      ...
      ...ejemplo(
        "Ejemplo 6 · No se puede recibir el ATIS (PLAIN LANGUAGE)",
      ...
      {
        kind: "hueco",
        rotulo: "CM-41-01 · Captura genérica · 4:5 · 1080×1350 px",
        descripcion:
          "Imagen sugerida: Impresión genérica de un D-ATIS de un aeródromo ficticio (Las Guindas) con la letra de identificación, hora, pista en uso, viento, visibilidad, nubes, temperatura y punto de rocío, QNH, nivel de transición y un aviso operacional. Resaltar con recuadros numerados: 1 la letra, 2 la pista, 3 el QNH, 4 el nivel de transición. Sin logotipos de operador ni de fabricante. Objetivo: Que el piloto ubique de un vistazo los cuatro datos que usará en la primera llamada y en la colación.",
        alto: 420,
        ratio: "4 / 5",
        anchoMax: 420,
      },
      ...
      {
        kind: "enLaOperacion",
        momento: "Antes de la puesta en marcha y antes del descenso",
        texto: "El ATIS se copia antes de la llamada de puesta en marcha y antes del descenso, porque con él se prepara la salida o la aproximación (pista, QNH, nivel de transición, avisos). Qué piloto lo copia y cómo se cruza lo fija el SOP del operador.",
        pasos: [
      ...
      error("Letra vieja", "**Letra vieja**: llamar con «information Bravo» cuando ya salió Charlie, con otro QNH o con otra pista. El controlador puede no notarlo en una frecuencia saturada."),
      error("Tomar la pista del ATIS como autorización", "**Tomar la pista del ATIS como autorización**: el ATIS dice la pista en uso; la pista asignada la da el controlador, y puede ser otra."),
      error("QNH copiado mal", "**QNH copiado mal** y no colacionado cuando el controlador lo repite."),
      error("No confirmar el ATIS", "**No confirmar el ATIS**: el controlador tiene que dar la información completa por voz y la frecuencia se carga."),
      error("Confundir hPa y pulgadas", "**Confundir hPa y pulgadas** cuando el ATIS de otro Estado usa otra unidad."),
      {
        kind: "summary",
      ...
      },
      {
        ...
        bloques: [
          ...
          {
            kind: "p",
            text: "Doc 4444 (15.ª ed., Enm. 4) cap. 1 (definiciones ATIS, ATIS-D, ATIS-voz), 4.5.7.5.1 c), 4.10.4.3; Doc 9432 (4.ª ed.) cap. 1, 2.8.2.2, 4.2.1, 4.2.2, 4.4.3, 4.6.2, 7.3.1.",
          },
  ...
  // ── 42 ──────────────────────────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      {
        kind: "p",
        text: "**VOLMET** es «información meteorológica para aeronaves en vuelo» (Doc 9432, cap. 1, abreviaturas). Es una radiodifusión: reportes y pronósticos de un grupo de aeródromos, repetidos en un horario fijo. No se habla con nadie; se escucha.",
      },
      { kind: "sub", text: "Lo que debe saber un piloto" },
      {
        kind: "list",
      ...
      verificar(
        "El contenido del VOLMET (METAR, TAF, SIGMET), sus horarios y la existencia de VOLMET HF, VHF y D-VOLMET no están en las fuentes cargadas: consultar el Anexo 3, cap. 11, el Anexo 11 y el Doc 9432 cap. 10. Estaciones y frecuencias VOLMET de cada región: AIP y documentación regional (ENR / GEN 3.5).",
      ),
      { kind: "sub", text: "Fraseología OACI" },
      {
        kind: "p",
        text: "VOLMET no tiene intercambio ATC/PILOT: es una emisión que solo se escucha. Por eso este capítulo no trae cuatro intercambios; trae cómo pedir el dato cuando el VOLMET no alcanza.",
      },
      COMO_LEER,
      ...ejemplo(
        "Ejemplo 1 · Pedir el tiempo a una estación aeronáutica (PLAIN LANGUAGE)",
      ...
      ...ejemplo(
        "Ejemplo 2 · Pedir la frecuencia VOLMET (PLAIN LANGUAGE)",
        [`PILOT: "Oceanic Radio, Aviatory 452, request VOLMET frequency."`],
        "Significado: pregunta breve; la estación da la frecuencia.",
      ),
      ...ejemplo(
        "Ejemplo 3 · Confirmar que el tiempo del destino cambió (PLAIN LANGUAGE)",
      ...
      {
        kind: "enLaOperacion",
        momento: "En un cruce oceánico",
        texto: "En un cruce oceánico la tripulación actualiza el tiempo del destino y de las alternativas en ruta. Lo hace por data link de la compañía o, si no hay, escuchando el VOLMET HF a la hora de emisión del aeródromo que le interesa. Qué fuentes usa y cada cuánto lo fija el operador.",
      },
      { kind: "sub", text: "Error frecuente" },
      error("Perder una llamada por escuchar el VOLMET", "Escuchar el VOLMET en la frecuencia de control y perder una llamada: se escucha con el segundo receptor, con SELCAL o con el otro piloto atento a la frecuencia de control."),
      error("Anotar el aeródromo equivocado", "Anotar el aeródromo equivocado porque la emisión trae varios seguidos."),
      error("Tomar el VOLMET como algo más que meteorología", "Tomar el VOLMET como autorización o como información ATC: es solo meteorología."),
      {
        kind: "summary",
        title: "En pocas palabras",
        items: [
      ...
      },
      {
        ...
        bloques: [
          { kind: "sub", text: "Verificado" },
          { kind: "p", text: "Doc 9432 (4.ª ed.) cap. 1 (abreviatura VOLMET), 2.6 (REQUEST)." },
          { kind: "sub", text: "Por verificar" },
          {
            kind: "list",
            items: [
  ...
  // ── 43 ──────────────────────────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      {
        kind: "p",
        text: "**CPDLC** (comunicaciones por enlace de datos controlador-piloto) es la «comunicación entre el controlador y el piloto por medio de enlace de datos para las comunicaciones ATC» (Doc 4444, cap. 1). En vez de hablar, controlador y piloto intercambian **mensajes de texto preformateados** que aparecen en una pantalla de la cabina y en la consola del controlador.",
      },
      {
        kind: "p",
        text: "Su guía operacional es el **GOLD** (Global Operational Data Link). Según la presentación de la FAA (2017), el GOLD reunió tres guías regionales: el manual de operaciones FANS 1/A de Asia-Pacífico, la guía de data link del Atlántico Norte y la guía LINK2000+ (ATN B1) de Europa. La 1.ª edición interregional la adoptaron Atlántico Norte y Asia-Pacífico en junio de 2010; Europa adoptó la 2.ª en abril de 2013. Después la OACI la publicó como **Doc 10037** (1.ª ed., 2017). Cubre DLIC, ADS-C y CPDLC sobre las tecnologías **FANS 1/A** y **ATN B1**.",
      },
      { kind: "sub", text: "Lo que debe saber un piloto" },
      {
        kind: "p",
        text: "**1. Logon (iniciación).** Antes de entrar en un espacio aéreo que exige data link, la aeronave y la dependencia ATS inician la comunicación por enlace de datos (Doc 4444, 4.15.1.1). Lo hace la aeronave (automático o por acción del piloto) o la dependencia ATS. La dirección de logon de cada dependencia se publica en la AIP (Doc 4444, 4.15.1.2). La función que intercambia direcciones y versiones se llama **DLIC** (Doc 4444, cap. 1). Si el logon falla, el sistema avisa a quien lo inició (Doc 4444, 4.15.4).",
      },
      { kind: "p", text: "**2. Quién te controla por data link.** El Doc 4444 (cap. 1) define:" },
      {
        kind: "glosario",
        items: [
          { k: "Autoridad de datos vigente", v: "la dependencia con la que tienes el diálogo CPDLC ahora." },
          { k: "Autoridad de datos siguiente", v: "la que la vigente designó para la transferencia." },
          { k: "Autoridad de datos ruta abajo", v: "otra dependencia con la que puedes comunicarte para recibir la autorización siguiente." },
        ],
      },
      {
        kind: "p",
        text: "Esto importa porque solo la autoridad vigente te controla. Tener logon con la siguiente no te pone bajo su control.",
      },
      { kind: "p", text: "**3. Uplink y downlink.**" },
      {
        kind: "glosario",
        items: [
          { k: "Uplink", v: "mensaje de tierra a la aeronave (autorización, instrucción, pregunta, información)." },
          { k: "Downlink", v: "mensaje de la aeronave a tierra (solicitud, respuesta, informe)." },
        ],
      },
      {
        kind: "p",
        text: "**4. Respuestas.** Cada uplink pide un tipo de respuesta. Las palabras son las mismas de la radiotelefonía y significan lo mismo (Doc 9432, 2.6):",
      },
      {
        kind: "table",
        head: ["Respuesta", "Significado (Doc 9432, 2.6)", "Cuándo se usa en CPDLC"],
        rows: [
      ...
      {
        kind: "p",
        text: "Qué respuesta admite cada mensaje lo fija el conjunto de mensajes (VERIFICAR abajo). La idea que sí es segura: **ROGER no es WILCO**. ROGER dice que lo recibiste; WILCO dice que lo vas a cumplir.",
      },
      { kind: "definicion", text: "ROGER no es WILCO." },
      {
        kind: "p",
        text: "**5. STANDBY no cierra nada.** Después de STANDBY todavía debes la respuesta final (WILCO o UNABLE).",
      },
      {
        kind: "p",
        text: "**6. Transferencia.** Al salir de un espacio aéreo, la autoridad vigente transfiere la conexión a la siguiente. La tripulación comprueba en la pantalla quién es ahora la autoridad vigente.",
      },
      {
        kind: "p",
        text: "**7. Performance.** El Doc 4444 (cap. 1) define la **RCP** (performance de comunicación requerida) y da como ejemplo el tipo **RCP 240**: valores de tiempo de transacción, continuidad, disponibilidad e integridad. Algunas separaciones reducidas exigen una RCP determinada. El marco completo (PBCS) está en el Doc 9869.",
      },
      {
        kind: "hueco",
        rotulo: "CM-43-02 · Diagrama · 16:9 · 1600×900 px",
        descripcion:
          "Imagen sugerida: Aeronave volando de izquierda a derecha entre dos FIR ficticias. Sobre la primera, rótulo «Autoridad de datos vigente» con línea sólida hacia el avión. Sobre la segunda, «Autoridad de datos siguiente» con línea punteada. En el límite de FIR, una flecha «transferencia» que convierte la punteada en sólida. Objetivo: Que el piloto entienda que solo la autoridad vigente lo controla y que la transferencia ocurre en el límite.",
        alto: 280,
      },
      { kind: "sub", text: "Fraseología OACI" },
      {
        kind: "p",
        text: "**Mensajes CPDLC.** Los textos de pantalla que siguen son **ilustrativos**. El texto exacto de cada mensaje está en el Doc 4444, apéndice 5, y en el Doc 10037, apéndice A, y **cambia entre FANS 1/A y ATN B1**. Ninguno se publica sin verificarlo.",
      },
      COMO_LEER,
      verificar(
        "Ningún texto de pantalla de esta lección está verificado («CLIMB TO FL350», «REQUEST CLIMB TO FL370», «CAN YOU ACCEPT FL370 AT TIMSA», «EXPECT CLIMB AT 1230», el motivo «DUE TO AIRCRAFT PERFORMANCE») ni el tipo de respuesta que admite cada uno: consultar el Doc 4444 16.ª ed., apéndice 5, y el Doc 10037, apéndice A, con sus diferencias entre FANS 1/A y ATN B1. Cierre de mensajes, tiempos de espera, texto libre y respuesta final después de STANDBY: Doc 4444 cap. 14 y Doc 10037 cap. 4. Las frases de voz «DISREGARD CPDLC … MESSAGE» y «ALL STATIONS CPDLC FAILURE»: Doc 4444 16.ª ed., cap. 12 y cap. 14. Transferencia de conexión y comprobación de la autoridad vigente: Doc 10037 cap. 4.",
      ),
      ...ejemplo(
        "Ejemplo 1 · Uplink con autorización de nivel (VERIFICAR texto)",
        [
      ...
      ...ejemplo(
        "Ejemplo 2 · Uplink que no se puede cumplir (VERIFICAR texto)",
        [
      ...
      ...ejemplo(
        "Ejemplo 3 · Solicitud del piloto (VERIFICAR texto)",
        [
      ...
      ...ejemplo(
        "Ejemplo 4 · Pregunta de sí o no (VERIFICAR texto y tipo de respuesta)",
        [
      ...
      ...ejemplo(
        "Ejemplo 5 · Mensaje informativo (VERIFICAR texto)",
        [
      ...
      {
        kind: "hueco",
        rotulo: "CM-43-01 · Esquema · 9:16 · 1080×1920 px",
        descripcion:
          "Imagen sugerida: Tres pantallas genéricas de CPDLC en secuencia vertical, sin marca de fabricante. 1 «UPLINK»: aparece un mensaje nuevo con la hora, la dependencia (ficticia) y el texto «CLIMB TO FL350» (texto a verificar antes de producir). 2 «REVIEW»: el mismo mensaje con una nota lateral «leer, cruzar con el otro piloto, seleccionar el nivel». 3 «RESPONSE»: botones WILCO, UNABLE y STANDBY, con WILCO resaltado y el estado «enviado». Flechas entre pantallas. Objetivo: Que el piloto vea que la respuesta va al final, después de leer, cruzar y seleccionar, y que las tres opciones significan cosas distintas.",
        alto: 560,
        ratio: "9 / 16",
        anchoMax: 360,
      },
      { kind: "p", text: "**Voz alrededor del CPDLC.** Cuando algo no está claro, se pasa a voz." },
      ...ejemplo(
        "Ejemplo 6 · Confirmar un mensaje dudoso (construido con palabras normalizadas del Doc 9432, 2.6; no es frase publicada)",
        [
      ...
      ...ejemplo(
        "Ejemplo 7 · Anulación de un mensaje CPDLC por voz (VERIFICAR)",
        [
      ...
      ...ejemplo(
        "Ejemplo 8 · Falla general de CPDLC anunciada por el ATC (VERIFICAR)",
      ...
      {
        kind: "enLaOperacion",
        momento: "Antes del espacio aéreo con data link y en crucero",
        texto: "Antes del espacio aéreo que lo exige, la tripulación hace el logon con la dirección publicada en la AIP o en la carta de ruta, y comprueba que la conexión quedó activa.",
        pasos: [
      ...
      { kind: "sub", text: "Error frecuente" },
      error("Enviar WILCO sin leer bien", "**Enviar WILCO sin leer bien** o sin cruzar con el otro piloto: es aceptar una autorización que no se comprobó."),
      error("Tratar STANDBY o AFFIRM como autorización", "**Tratar STANDBY o AFFIRM como autorización.** Una pregunta («can you accept») no es una autorización."),
      error("Responder ROGER a una autorización", "**Responder ROGER a una autorización**: el controlador queda sin saber si la vas a cumplir."),
      error("Contestar tarde o no contestar", "**Contestar tarde o no contestar**: el mensaje queda abierto y el controlador puede terminar llamando por voz."),
      error("Ejecutar un uplink que contradice la voz", "**Ejecutar un uplink que contradice una instrucción de voz reciente** sin aclararlo."),
      error("Perder la transferencia", "**Perder la transferencia**: seguir enviando solicitudes a la dependencia anterior."),
      {
        kind: "summary",
        title: "En pocas palabras",
        items: [
