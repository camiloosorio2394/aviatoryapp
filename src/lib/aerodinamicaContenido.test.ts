...
describe("Aerodinámica: las doce secciones", () => {
  ...
  it("lleva dos figuras didácticas completas y distintas en cada lección", () => {
    ...
    for (const f of figuras) {
      ...
      // Sin tope: la figura va al ancho de la columna de lectura, que es la
      // medida estándar de las fotos del curso (BLOQUES_ANCHOS en el lector).
      expect(f.anchoMax, f.src).toBeUndefined()
    }
  })
})
