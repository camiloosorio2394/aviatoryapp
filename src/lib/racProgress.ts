/**
 * Progreso del módulo RAC: el progreso de módulo común (progresoModulo) con el
 * respaldo local del tema, su RPC rac_mark_progress y la mejor nota de la
 * evaluación.
 *
 * Mismo patrón que Performance y Aeropuertos: lo local primero, la base como
 * verdad entre dispositivos, y lo avanzado sin sesión se sube cuando aparece
 * una.
 *
 * Mientras no se corra supabase/migrations/20260928000000_modulos_rac_y_combustible.sql
 * la tabla no existe: la consulta falla, `leer` devuelve null y el módulo sigue
 * contando con lo del navegador. El día que se corra, lo que cada piloto tenga
...
const LS_KEY = "aviatory.rac.progress"

export interface RacProgreso {
  /** Números de unidad leída, 1 a RAC_LECTURA_TOTAL. */
  lessonScreens: number[]
  /** Claves de práctica hechas: "u05-q2", "u10-q1"… */
  practiceDone: string[]
  /** Mejor puntaje de la evaluación, sobre 100. */
  bestScore: number | null
}

/** Lo que devuelve fetchRacProgress: lo unido para mostrar, y lo que la base tiene de verdad. */
export interface RacTraido extends RacProgreso {
  remoto: ProgresoRemoto
}

const VACIO: RacProgreso = { lessonScreens: [], practiceDone: [], bestScore: null }

export function readRacLocal(): RacProgreso {
  try {
    const raw = localStorage.getItem(LS_KEY)
    if (!raw) return VACIO
    const p = JSON.parse(raw) as Partial<RacProgreso>
    return {
...
export function writeRacLocal(patch: Partial<RacProgreso>): void {
  try {
    localStorage.setItem(LS_KEY, JSON.stringify({ ...readRacLocal(), ...patch }))
  } catch {
...
const progreso = crearProgresoModulo({
  tabla: "user_rac_progress",
  rpc: "rac_mark_progress",
  leerLocal: readRacLocal,
  anotarLocal: (marca) => {
    const antes = readRacLocal()
 ...
 */
export async function fetchRacProgress(userId: string): Promise<RacTraido | null> {
  const [remoto, examen] = await Promise.all([
    progreso.leer(userId),
    supabase
      .from("user_rac_exam_attempts")
      .select("score")
      .eq("user_id", userId)
...
/** Marca una unidad leída o una pregunta de práctica respondida, local y en la base. */
export const markRacProgress = progreso.marcar

/**
 * Sube lo que se avanzó sin sesión, comparando contra lo que la base tiene.
 * Devuelve lo de la base más lo que se subió bien.
 */
export async function pushPendingRac(traido: RacTraido): Promise<RacProgreso> {
