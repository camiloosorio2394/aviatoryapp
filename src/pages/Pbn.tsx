...
const ACENTO = "var(--av-pbn-700)"
const ACENTO_CLARO = "var(--av-pbn-500)"
...
export function Pbn() {
  ...
  const [progreso, setProgreso] = useState(() => readPbnLocal())
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        const traido = await fetchPbnProgress(user.id)
  ...
  const resumen = useMemo(() => resumirPbn(progreso), [progreso])
  ...
  const partes: CourseCardProps[] = [
    {
      ...
      status:
        resumen.lessonRead === 0
          ? "Sin empezar"
          : resumen.lessonRead >= PBN_LECTURA_TOTAL
            ? "Lección completa"
            : `${resumen.lessonRead} de ${PBN_LECTURA_TOTAL} capítulos leídos`,
      progress: resumen.lessonPct,
      done: resumen.lessonRead >= PBN_LECTURA_TOTAL,
    },
    {
      to: PBN_PRACTICA_RUTA,
      densidad: "compacta" as const,
      photoAspect: "5/2" as const,
      icon: Target,
      color: ACENTO,
      ...
      status:
        resumen.practiceDone === 0
          ? "Sin empezar"
          : `${resumen.practiceDone} de ${PBN_PRACTICA_TOTAL} respondidas`,
      progress: resumen.practicePct,
      done: resumen.practiceDone >= PBN_PRACTICA_TOTAL,
    },
    {
      to: PBN_EVALUACION,
      densidad: "compacta" as const,
          ...
          <div className="self-start overflow-hidden rounded-[14px] border border-white/15 bg-[rgba(6,17,31,0.62)] backdrop-blur-[6px] lg:mt-[33px] lg:min-w-[210px]">
              ...
              {loading ? (
                ...
                <>
                  ...
                  <div
                    ...
                    aria-valuemin={0}
                    aria-valuemax={100}
                    aria-label="Avance del módulo PBN"
                  >
                    <div
            ...
            <div className="border-t border-white/10 p-1">
              <FilaAvance
                titulo="Lección"
                to={PBN_APRENDE}
                valor={`${resumen.lessonRead} / ${PBN_LECTURA_TOTAL}`}
              ...
              <FilaAvance
                titulo="Práctica"
                to={PBN_PRACTICA_RUTA}
                valor={`${resumen.practiceDone} / ${PBN_PRACTICA_TOTAL}`}
