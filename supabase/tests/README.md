...
| `evaluacion_temas.sql` | La evaluación entrega el tema de cada pregunta (el de la fuente, o si no, el de los metadatos): Aerodinámica, Performance, RAC y Combustible con tema en cada pregunta, NOTAM sin tema y el simulacro con su etiqueta, como antes. Y RAC y Combustible como el repo: lección exigida en el servidor (18 de 19 no abre) y el catálogo de Combustible con los diez escenarios. Con `20260930000000` aplicada. |
| `pbn.sql` | El módulo PBN de la migración `20260929120000`: catálogo (48 lecciones, claves `pNN-qN`), reglas de la evaluación (20 de 66, aprueba con 80, corrección al final, un solo banco, lección exigida), el banco completo y con tema, RPC idempotente y `security definer`, RLS, sin escritura directa ni sin sesión, `anon` sin nada, la puerta (47 de 48 no abre) y el intento que llega a su tabla, los cuatro logros; después, que los diez módulos de antes siguen contando, `evaluacion_terminar` con todos sus destinos y el panel con los once. Con la migración aplicada y el banco sembrado. |
| `mel.sql` | Progreso de MEL contra su catálogo (40 lecciones y las claves de práctica de `contenido/catalogo`), RPC idempotente y `security definer`, RLS, sin escritura directa ni sin sesión, `anon` sin nada, la puerta de la evaluación (con 39 lecciones no abre, con 40 sí) y el intento que llega a su tabla, los conteos de los otros once módulos, el panel con los doce módulos, el plan y las postulaciones, y los cuatro logros. Con las tres migraciones de MEL (`20261001000000`, `010000` y `020000`), el catálogo cargado y el banco sembrado. |
| `mel_evaluacion.sql` | Las reglas de la evaluación de MEL (25 de 70, aprueba con 80, corrección al final, un solo banco, lección exigida), el banco completo y repartido por nivel, la tabla de intentos cerrada, que `evaluacion_terminar` siga enrutando a los trece destinos (doce módulos y el simulacro) y que `secciones_leidas` siga contando los doce módulos. Con las tres migraciones y el banco sembrado. |
| `autorizaciones_y_eliminacion.sql` | La autorización se guarda una vez por documento y versión y cada piloto lee solo las suyas; un certificado médico no se guarda sin la autorización de dato sensible; `eliminar_mi_cuenta` exige la palabra, no la ejecuta anon ni alguien sin sesión, y borra la cuenta del que llama con todo lo suyo (también el registro de acceso al banco) sin tocar a nadie más. Borra dentro de la transacción y la deshace. Con `20261002000000` aplicada. |
| `topes_y_restricciones.sql` | Publicar 30 mensajes, borrarlos y publicar otro sigue topado; el registro de inserciones no lo lee ni lo borra el cliente; las seis tablas nuevas tienen su tope; una reacción larga, una evidencia en la carpeta de otro piloto y una foto de otro sitio se rechazan, y las válidas pasan. Con `20261002010000` aplicada (después de `20261002000000`). |
| `foro.sql` | El cliente no toca las tablas del foro y anon no ejecuta nada; publicar nace con el voto del autor y da «Hola comunidad»; votar por diferencia; comentar suma, avisa al autor una vez y respeta dos niveles; un aviso pide aerolínea y su autor no lo confirma; reportar cuenta una vez por piloto; lo que las lecturas devuelven sin sesión, para cuando se abra; borrar lo saca del feed; tope de 10 publicaciones al día. Con `20261003000000` y `20261003010000` aplicadas. |
...
| `logros.sql` | Lección y práctica contra el catálogo (NOTAM, Aeropuertos, Comunicaciones ATC, RAC, Gestión del combustible y MEL), claves viejas que no cuentan, un grupo por disparador, ponerse al día solo sobre sí mismo. Desde el caso de Aeropuertos necesita `20260916000000` aplicada, desde el de Comunicaciones, `20260927020000`, desde los de RAC y Combustible, `20260928000000`, y desde el de MEL, `20261001020000`. |
...
| `constancia.sql` | El plan de estudio es del piloto y solo suyo, la racha sobrevive a un día por mes (uno, no dos), y los avisos que corren de noche salen cuando deben y una sola vez al día. La meta de horas se mide contra una convocatoria abierta (desde `20261001050000`), con las horas que ya trae la fila (desde `20261001060000`). |
