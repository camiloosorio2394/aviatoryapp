...
| `mel.sql` | Progreso de MEL contra su catálogo (40 lecciones, ninguna práctica todavía), RPC idempotente y `security definer`, RLS, sin escritura directa ni sin sesión, `anon` sin nada, y que los otros siete módulos conservan su catálogo. Con `20260928000000` aplicada. |
...
| `logros.sql` | Lección y práctica contra el catálogo (NOTAM, Aeropuertos y Comunicaciones ATC), claves viejas que no cuentan, un grupo por disparador, ponerse al día solo sobre sí mismo. Desde el caso de Aeropuertos necesita `20260916000000` aplicada, y desde el de Comunicaciones, `20260927020000`. |
