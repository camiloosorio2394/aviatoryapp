...
| `mel.sql` | Progreso de MEL contra su catálogo (40 lecciones y las claves de práctica de `contenido/catalogo`), RPC idempotente y `security definer`, RLS, sin escritura directa ni sin sesión, `anon` sin nada, la puerta de la evaluación (con 39 lecciones no abre, con 40 sí) y el intento que llega a su tabla, los conteos de los otros siete módulos, el panel con los ocho módulos, el plan y las postulaciones, y los cuatro logros. Con las tres migraciones de MEL (`20260928000000`, `010000` y `020000`), el catálogo cargado y el banco sembrado. |
| `mel_evaluacion.sql` | Las reglas de la evaluación de MEL (25 de 70, aprueba con 80, corrección al final, un solo banco, lección exigida), el banco completo y repartido por nivel, la tabla de intentos cerrada, que `evaluacion_terminar` siga enrutando a los nueve destinos (ocho módulos y el simulacro) y que `secciones_leidas` siga contando los ocho módulos. Con las tres migraciones y el banco sembrado. |
...
| `logros.sql` | Lección y práctica contra el catálogo (NOTAM, Aeropuertos, Comunicaciones ATC y MEL), claves viejas que no cuentan, un grupo por disparador, ponerse al día solo sobre sí mismo. Desde el caso de Aeropuertos necesita `20260916000000` aplicada, desde el de Comunicaciones, `20260927020000`, y desde el de MEL, `20260928020000`. |
