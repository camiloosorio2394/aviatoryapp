...
begin
  ...
  on conflict (user_id) do update
    set lesson_screens = excluded.lesson_screens, practice_done = excluded.practice_done;
  if exists (select 1 from public.user_achievements ua join public.achievements a on a.id = ua.achievement_id
             where ua.user_id = x_b and a.code in ('comunicaciones_lesson', 'comunicaciones_practice')) then
    raise exception 'FALLO comunicaciones ganó logros con 68 lecciones y claves viejas';
  end if;
  update public.user_comunicaciones_progress
  set lesson_screens = (select array_agg(i::smallint) from generate_series(1, 69) i)
  where user_id = x_b;
  if not exists (select 1 from public.user_achievements ua join public.achievements a on a.id = ua.achievement_id
                 where ua.user_id = x_b and a.code = 'comunicaciones_lesson') then
    raise exception 'FALLO comunicaciones_lesson no se ganó con las 69 lecciones';
  end if;
  x_log := x_log || ' comunicaciones_con_catalogo';

  -- MEL (20260928020000): igual, contra su catálogo de 40. Y la práctica no
