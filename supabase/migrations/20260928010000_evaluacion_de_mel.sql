...
-- (`supabase migration repair --status applied 20260928010000`, ver
-- docs/MEL_ESTADO.md).
--
...
--      repo (20260927010000, con los siete módulos y el simulacro), con la
--      rama `when 'mel'` añadida. Sin esa rama el CASE no encuentra el
...
--      20260927010000) gana la rama de user_mel_progress.
--
...
--     20260928020000_panel_y_logros_de_mel.sql, que lee la tabla de intentos
--     que nace aquí.
--
-- ORDEN: DESPUÉS de 20260928000000_progreso_de_mel.sql, que crea
...
-- (hasta 20260927020000) ya está en producción.
--
...
-- La lista es la de 20260927010000 (los siete módulos y el simulacro) más
...
alter table public.evaluaciones add constraint evaluaciones_destino_check
  check (destino in ('notam', 'metar', 'mercancias', 'aerodinamica', 'aeropuertos', 'performance', 'comunicaciones', 'mel', 'simulacro_aerolinea'));
...
-- (20260928020000) comparan contra esto y no contra un 80 a mano.
...
--
-- La función entera, idéntica a la de 20260927010000 (la última que la
-- publica, con los siete módulos y el simulacro), más el `when 'mel'`. No
-- cambia nada más.
...
--
-- La de 20260927010000 (los siete módulos) con una rama más.
