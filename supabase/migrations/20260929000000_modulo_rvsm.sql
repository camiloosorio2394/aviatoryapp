-- ============================================================================
-- RVSM: el módulo completo
--
-- Trae la tabla de progreso con su RPC, la fila del catálogo (que es lo que la
-- base acepta como capítulo y como práctica), los umbrales, la tabla de
-- intentos, las reglas de la evaluación con su banco, los cuatro logros y sus
-- disparadores.
--
-- Y republica las SEIS funciones compartidas. No están copiadas a mano: salen
-- enteras de 20260928000000, que es la última aplicada según la marca del
-- historial, con la rama de RVSM insertada encima. Republicarlas a ojo fue lo
-- que se llevó por delante `plan` y `postulaciones` en septiembre.
--
...
-- ── Progreso del módulo ──
create table if not exists public.user_rvsm_progress (
...
alter table public.user_rvsm_progress enable row level security;

-- Solo lectura de lo propio. Escribir es exclusivo de la RPC, que valida
-- contra el catálogo.
drop policy if exists rvsm_progress_select_own on public.user_rvsm_progress;
create policy rvsm_progress_select_own
  on public.user_rvsm_progress
  for select
  to authenticated
  using ((select auth.uid()) = user_id);

revoke all on table public.user_rvsm_progress from anon, authenticated;
grant select on table public.user_rvsm_progress to authenticated;

-- Copia de rac_mark_progress: valida contra el catálogo con
-- private.validar_marca_progreso y agrega sin duplicar.
create or replace function public.rvsm_mark_progress(
  p_lesson_screen smallint default null,
  p_practice_id text default null
)
returns public.user_rvsm_progress
language plpgsql
security definer
set search_path to ''
as $function$
declare
  v_row public.user_rvsm_progress;
begin
  if auth.uid() is null then
    raise exception 'no authenticated user';
  end if;
  perform private.validar_marca_progreso('rvsm', p_lesson_screen, p_practice_id);

  insert into public.user_rvsm_progress (user_id, lesson_screens, practice_done, updated_at)
  values (
    auth.uid(),
  ...
  on conflict (user_id) do update set
    lesson_screens = (
      ...
      from unnest(
        public.user_rvsm_progress.lesson_screens ||
        case when p_lesson_screen is null then '{}'::smallint[] else array[p_lesson_screen] end
      ) as s
    ),
    practice_done = (
      ...
      from unnest(
        public.user_rvsm_progress.practice_done ||
        case when p_practice_id is null then '{}'::text[] else array[p_practice_id] end
      ) as p
    ),
    updated_at = now()
...
revoke all on function public.rvsm_mark_progress(smallint, text) from public, anon;
grant execute on function public.rvsm_mark_progress(smallint, text) to authenticated;

-- ── Intentos de la evaluación ──
create table if not exists public.user_rvsm_exam_attempts (
...
create index if not exists user_rvsm_exam_attempts_user_idx
  on public.user_rvsm_exam_attempts (user_id, taken_at desc);

alter table public.user_rvsm_exam_attempts enable row level security;

drop policy if exists rvsm_exam_select_own on public.user_rvsm_exam_attempts;
create policy rvsm_exam_select_own
  on public.user_rvsm_exam_attempts
  for select
  to authenticated
  using ((select auth.uid()) = user_id);
...
-- solo lee lo suyo.
revoke all on table public.user_rvsm_exam_attempts from anon, authenticated;
grant select on table public.user_rvsm_exam_attempts to authenticated;

-- ── El catálogo de contenido ──
-- Las claves salen de scripts/rvsm/convertir.mjs, no de una lista a mano.
insert into public.modulos_contenido (modulo, lecciones, practicas)
values ('rvsm', 32, array['r01-q1', 'r01-q2', 'r01-q3', 'r02-q1', 'r02-q2', 'r02-q3', 'r03-q1', 'r03-q2', 'r03-q3', 'r04-q1', 'r04-q2', 'r04-q3', 'r05-q1', 'r05-q2', 'r05-q3', 'r06-q1', 'r06-q2', 'r06-q3', 'r07-q1', 'r07-q2', 'r07-q3', 'r08-q1', 'r08-q2', 'r08-q3', 'r09-q1', 'r09-q2', 'r09-q3', 'r10-q1', 'r10-q2', 'r10-q3', 'r11-q1', 'r11-q2', 'r11-q3', 'r12-q1', 'r12-q2', 'r12-q3', 'r13-q1', 'r13-q2', 'r13-q3', 'r14-q1', 'r14-q2', 'r14-q3', 'r15-q1', 'r15-q2', 'r15-q3', 'r16-q1', 'r16-q2', 'r16-q3', 'r17-q1', 'r17-q2', 'r17-q3', 'r18-q1', 'r18-q2', 'r18-q3', 'r19-q1', 'r19-q2', 'r19-q3', 'r20-q1', 'r20-q2', 'r20-q3', 'r21-q1', 'r21-q2', 'r21-q3', 'r22-q1', 'r22-q2', 'r22-q3', 'r23-q1', 'r23-q2', 'r23-q3', 'r24-q1', 'r24-q2', 'r24-q3', 'r25-q1', 'r25-q2', 'r25-q3', 'r26-q1', 'r26-q2', 'r26-q3', 'r27-q1', 'r27-q2', 'r27-q3', 'r28-q1', 'r28-q2', 'r28-q3', 'r29-q1', 'r29-q2', 'r29-q3', 'r30-q1', 'r30-q2', 'r30-q3', 'r31-q1', 'r31-q2', 'r31-q3', 'r32-q1', 'r32-q2', 'r32-q3'])
...
insert into public.module_thresholds (code, total, nota) values
  ('rvsm_lesson', 32, 'RVSM_LECTURA_TOTAL de src/lib/rvsm.ts (32 capítulos en 6 bloques)'),
  ('rvsm_pass', 80, 'RVSM_PASS_SCORE: 20 preguntas al azar de 40, apruebas con 80')
...
alter table public.evaluaciones add constraint evaluaciones_destino_check
  check (destino = any (array[
    ...
    'performance', 'comunicaciones', 'rac', 'combustible', 'rvsm',
    'simulacro_aerolinea'
  ]));
...
values
  ('rvsm_evaluacion', 'RVSM · Evaluación', 'al_final', 20, 80,
   true, 180, 'rvsm', true, 'rvsm')
...
-- De un solo banco y sin cupo: el intento se sortea entero sobre las 40.
insert into public.evaluacion_fuentes (evaluacion, banco, etiqueta, cupo)
values ('rvsm_evaluacion', 'rvsm_evaluacion', null, null)
on conflict (evaluacion, banco) do update set
  etiqueta = excluded.etiqueta,
  cupo = excluded.cupo;

-- ── Los cuatro logros ──
-- 42 a 45: los anteriores llegan hasta el 41 (Gestión del combustible).
insert into public.achievements (code, name, description, icon, tier, order_index) values
  ('rvsm_lesson', 'RVSM leído', 'Leíste los treinta y dos capítulos del módulo RVSM', '📖', 'bronze', 42),
  ('rvsm_practice', 'RVSM practicado', 'Respondiste las noventa y seis preguntas de práctica del módulo RVSM', '📐', 'silver', 43),
  ('rvsm_exam', 'Evaluación de RVSM superada', 'Aprobaste la evaluación del módulo RVSM', '✅', 'silver', 44),
  ('rvsm_master', 'RVSM dominado', 'Terminaste el módulo entero: lección, práctica y evaluación', '🛫', 'gold', 45)
...
begin
  ...
  foreach v_grupo in array array[
    ...
    'notam', 'metar', 'aerolinea', 'mercancias', 'aerodinamica', 'aeropuertos', 'performance', 'comunicaciones', 'rac', 'combustible', 'rvsm'
  ] loop
