-- ============================================================================
-- Módulos RAC y Gestión del combustible: progreso, catálogo, evaluación,
-- panel y logros, los dos en una sola migración.
--
-- NO SE HA APLICADO. Se entrega para correrla aparte (Nico tiene el editor SQL
-- desde el 13-sep-2026), y después el archivo se renombra con la versión que
-- registró la base:
--   select version from supabase_migrations.schema_migrations
--   where name = 'modulos_rac_y_combustible';
--
-- Por qué una sola y no una por módulo: seis funciones se republican enteras en
-- cada módulo y la última que corre decide qué sobrevive
-- (scripts/migraciones/funciones-compartidas.test.ts). Con dos migraciones
-- hermanas, la segunda tendría que copiar de la primera; con una, las dos
-- ramas entran juntas y no hay orden que equivocar.
--
-- Lo que hace, por módulo ('rac' y 'combustible'):
--   1. La tabla de progreso (solo select para el cliente), su fila en
--      modulos_contenido con las lecciones y las claves de práctica de
--      contenido/catalogo/modulos.json, el umbral de lección y la RPC
--      <modulo>_mark_progress, que valida contra el catálogo.
--   2. La tabla de intentos, las reglas de la evaluación (20 preguntas al azar,
--      aprueba con 80, corrección al final, opciones barajadas, 3 horas), de
--      qué banco sale y el umbral de aprobación.
--   3. Las seis funciones compartidas, copiadas de su versión más reciente (las
--      de Comunicaciones, 20260927010000 y 20260927020000) con la rama de cada
--      módulo: evaluacion_terminar, secciones_leidas, practicas_hechas,
--      desbloquear_logros, check_and_unlock_achievements y panel_tarjetas.
--   4. Los cuatro logros de cada módulo y sus dos disparadores.
--
-- Las preguntas NO van aquí: los bancos se editan en los documentos
-- (docs/contenido/rac.md y docs/contenido/gestion-combustible.md), los pasan a
-- contenido/bancos/ scripts/rac/convertir.mjs y scripts/combustible/convertir.mjs,
-- y se cargan con supabase/seeds/rac_evaluacion.sql y
-- supabase/seeds/combustible_evaluacion.sql (salida de
-- node scripts/bancos/sembrar.mjs).
--
-- ORDEN: DESPUÉS de las tres de Comunicaciones (20260927000000, 010000 y
-- 020000), de las que copia las funciones compartidas. Corrida antes, el panel
-- y los logros perderían Comunicaciones.
--
-- Se puede correr dos veces: todo es `if not exists`, `on conflict`,
-- `drop … if exists` o `create or replace`.
--
-- Prueba: supabase/tests/rac_y_combustible.sql, con el banco sembrado, y los
-- casos de los dos módulos en supabase/tests/logros.sql.
--
-- GENERADO por el script de la integración a partir de las migraciones de
-- Comunicaciones: no se editan las funciones a mano en este archivo.
-- ============================================================================

-- ── 1 · Progreso ───────────────────────────────────────────────────────────

-- ── 1.a · RAC: progreso, catálogo, umbral y RPC ──────────
...
insert into public.module_thresholds (code, total, nota) values
  ('rac_lesson', 19, 'RAC_LECTURA_TOTAL de src/lib/rac.ts (una unidad por reglamento, en 5 bloques)')
...
-- ── 1.b · Gestión del combustible: progreso, catálogo, umbral y RPC ──────────
...
insert into public.module_thresholds (code, total, nota) values
  ('combustible_lesson', 23, 'CB_LECTURA_TOTAL de src/lib/combustible.ts (23 capítulos en 3 partes)')
on conflict (code) do update set total = excluded.total, nota = excluded.nota;
...
-- ── 2 · Evaluación ─────────────────────────────────────────────────────────
--
-- Mismas columnas que los demás módulos, que es lo que el historial de la
-- pantalla sabe leer. Quien inserta es evaluacion_terminar (security definer).
...
-- El CHECK de destino se amplía antes de insertar las reglas: la lista es la de
-- 20260927010000 más 'rac' y 'combustible'.
...
alter table public.evaluaciones add constraint evaluaciones_destino_check
  check (destino in ('notam', 'metar', 'mercancias', 'aerodinamica', 'aeropuertos', 'performance', 'comunicaciones', 'rac', 'combustible', 'simulacro_aerolinea'));
...
values ('rac_evaluacion', 'rac_evaluacion', null, null)
on conflict (evaluacion, banco) do update set
  etiqueta = excluded.etiqueta,
  cupo = excluded.cupo;

insert into public.module_thresholds (code, total, nota) values
  ('rac_pass', 80, 'RAC_PASS_SCORE: 20 preguntas al azar de 50, apruebas con 80')
on conflict (code) do update set total = excluded.total, nota = excluded.nota;
...
values
  ('combustible_evaluacion', 'Gestión del combustible · Evaluación', 'al_final', 20, 80,
   true, 180, 'combustible', true, 'combustible')
on conflict (clave) do update set
  titulo = excluded.titulo,
  retroalimentacion = excluded.retroalimentacion,
  preguntas_por_intento = excluded.preguntas_por_intento,
...
insert into public.evaluacion_fuentes (evaluacion, banco, etiqueta, cupo)
values ('combustible_evaluacion', 'combustible_evaluacion', null, null)
on conflict (evaluacion, banco) do update set
  etiqueta = excluded.etiqueta,
  cupo = excluded.cupo;

insert into public.module_thresholds (code, total, nota) values
  ('combustible_pass', 80, 'CB_PASS_SCORE: 20 preguntas al azar de 40, apruebas con 80')
on conflict (code) do update set total = excluded.total, nota = excluded.nota;

-- ── 3 · evaluacion_terminar, con las dos ramas ─────────────────────────────
...
-- ── 4 · Contar lo leído y lo practicado ────────────────────────────────────
...
insert into public.achievements (code, name, description, icon, tier, order_index) values
  ('rac_lesson', 'RAC leído', 'Leíste las diecinueve unidades del módulo RAC', '📖', 'bronze', 34),
  ('rac_practice', 'RAC practicado', 'Respondiste las cincuenta y cuatro preguntas de práctica del módulo RAC', '⚖️', 'silver', 35),
  ('rac_exam', 'Evaluación de RAC superada', 'Aprobaste la evaluación del módulo RAC', '✅', 'silver', 36),
  ('rac_master', 'RAC dominado', 'Terminaste el módulo entero: lección, práctica y evaluación', '🏛️', 'gold', 37),
  ('combustible_lesson', 'Combustible leído', 'Leíste los veintitrés capítulos de Gestión del combustible', '📖', 'bronze', 38),
  ('combustible_practice', 'Combustible practicado', 'Hiciste toda la práctica: las preguntas de los capítulos y los diez escenarios', '⛽', 'silver', 39),
  ('combustible_exam', 'Evaluación de combustible superada', 'Aprobaste la evaluación de Gestión del combustible', '✅', 'silver', 40),
  ('combustible_master', 'Combustible dominado', 'Terminaste el módulo entero: lección, práctica y evaluación', '🛬', 'gold', 41)
...
create trigger trg_check_achievements_rac_exam
  after insert or update on public.user_rac_exam_attempts
  for each row execute function private.trigger_logros('rac');
...
create trigger trg_check_achievements_combustible_exam
  after insert or update on public.user_combustible_exam_attempts
  for each row execute function private.trigger_logros('combustible');

-- ── 8 · Los grupos del repaso manual ──────────────────────────────────────
...
-- ── 9 · El panel, con las dos tarjetas ────────────────────────────────────
