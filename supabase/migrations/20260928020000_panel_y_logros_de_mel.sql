...
-- (`supabase migration repair --status applied 20260928020000`, ver
-- docs/MEL_ESTADO.md).
--
...
-- más reciente, que es la de 20260927020000 (aplicada en producción, con los
-- siete módulos), y lo único que cambia en cada una es la rama de 'mel'.
-- panel_tarjetas conserva 'plan', 'postulaciones' y los siete módulos de
-- antes. scripts/migraciones/funciones-compartidas.test.ts lo comprueba.
--
-- ORDEN: DESPUÉS de 20260928010000_evaluacion_de_mel.sql, porque el panel y
...
-- dejó 20260928000000: la RPC rechaza toda clave y el logro de práctica no se
-- puede ganar (ver la guarda en desbloquear_logros). Nada se rompe.

-- ── 2 · Contar lo practicado ───────────────────────────────────────────────
--
-- La de 20260927020000 (los siete módulos) con una rama más. (secciones_leidas
-- ya la ganó en 20260928010000, para la puerta de la evaluación.)
...
-- Orden 34 a 37: después de los de Comunicaciones (30 a 33).
insert into public.achievements (code, name, description, icon, tier, order_index) values
  ('mel_lesson', 'MEL leída', 'Leíste las cuarenta lecciones del módulo de MEL', '📖', 'bronze', 34),
  ('mel_practice', 'MEL practicada', 'Resolviste todos los ejercicios de la práctica con entradas de MEL', '📋', 'silver', 35),
  ('mel_exam', 'Evaluación de MEL superada', 'Aprobaste la evaluación de MEL', '✅', 'silver', 36),
  ('mel_master', 'MEL dominada', 'Terminaste el módulo entero: lección, práctica y evaluación', '🛠️', 'gold', 37)
...
--
-- La de 20260927020000 (los siete módulos) con el grupo 'mel' agregado. La
...
-- siembra del catálogo, y el de 20260928000000 corrida otra vez).
...
begin
  ...
  foreach v_grupo in array array[
    ...
    'notam', 'metar', 'aerolinea', 'mercancias', 'aerodinamica', 'aeropuertos', 'performance', 'comunicaciones', 'mel'
  ] loop
...
--
-- La de 20260927020000 (plan, postulaciones y los siete módulos) con la
-- tarjeta de 'mel'. No se quita nada.
