-- ════════════════════════════════════════════════════════════════════════════
-- Módulo PBN · Performance-Based Navigation
--
-- El módulo once de preparación de aerolínea. Trae lo propio del tema (las dos
-- tablas del piloto, la RPC de progreso, el catálogo, los umbrales, la
-- evaluación y los cuatro logros) y republica enteras las seis funciones
-- compartidas con la rama de pbn añadida.
--
-- Las seis son private.secciones_leidas, private.practicas_hechas,
-- private.desbloquear_logros, public.check_and_unlock_achievements,
-- public.evaluacion_terminar y public.panel_tarjetas. En la base manda la
-- última que se corre, así que esta migración las deja nombrando los once
-- módulos del catálogo.
--
-- La versión va entre la última aplicada (20260929000000) y la que está
-- pendiente de correr (20260930000000), para no dejar un archivo pendiente por
-- debajo de la marca de supabase/HISTORIAL_DE_MIGRACIONES.md.
--
-- El contenido lo genera scripts/pbn/convertir.mjs desde docs/contenido/pbn.md,
-- y el banco de la evaluación se siembra aparte con scripts/bancos/sembrar.mjs.
-- ════════════════════════════════════════════════════════════════════════════

-- ── Progreso del módulo ──
create table if not exists public.user_pbn_progress (
...
alter table public.user_pbn_progress enable row level security;

-- Solo lectura de lo propio. Escribir es exclusivo de la RPC, que valida
-- contra el catálogo.
drop policy if exists pbn_progress_select_own on public.user_pbn_progress;
create policy pbn_progress_select_own
  on public.user_pbn_progress
  for select
  to authenticated
  using ((select auth.uid()) = user_id);

revoke all on table public.user_pbn_progress from anon, authenticated;
grant select on table public.user_pbn_progress to authenticated;

-- Copia de rac_mark_progress: valida contra el catálogo con
-- private.validar_marca_progreso y agrega sin duplicar.
create or replace function public.pbn_mark_progress(
  p_lesson_screen smallint default null,
  p_practice_id text default null
)
returns public.user_pbn_progress
language plpgsql
security definer
set search_path to ''
as $function$
declare
  v_row public.user_pbn_progress;
begin
  if auth.uid() is null then
    raise exception 'no authenticated user';
  end if;
  perform private.validar_marca_progreso('pbn', p_lesson_screen, p_practice_id);

  insert into public.user_pbn_progress (user_id, lesson_screens, practice_done, updated_at)
  values (
    auth.uid(),
  ...
  on conflict (user_id) do update set
    lesson_screens = (
      ...
      from unnest(
        public.user_pbn_progress.lesson_screens ||
        case when p_lesson_screen is null then '{}'::smallint[] else array[p_lesson_screen] end
      ) as s
    ),
    practice_done = (
      ...
      from unnest(
        public.user_pbn_progress.practice_done ||
        case when p_practice_id is null then '{}'::text[] else array[p_practice_id] end
      ) as p
    ),
    updated_at = now()
...
revoke all on function public.pbn_mark_progress(smallint, text) from public, anon;
grant execute on function public.pbn_mark_progress(smallint, text) to authenticated;

-- ── Intentos de la evaluación ──
create table if not exists public.user_pbn_exam_attempts (
...
create index if not exists user_pbn_exam_attempts_user_idx
  on public.user_pbn_exam_attempts (user_id, taken_at desc);

alter table public.user_pbn_exam_attempts enable row level security;

drop policy if exists pbn_exam_select_own on public.user_pbn_exam_attempts;
create policy pbn_exam_select_own
  on public.user_pbn_exam_attempts
  for select
  to authenticated
  using ((select auth.uid()) = user_id);
...
insert into public.module_thresholds (code, total, nota) values
  ('pbn_lesson', 52, 'PBN_LECTURA_TOTAL de src/lib/pbn.ts (52 capítulos en 9 bloques)'),
  ('pbn_pass', 80, 'PBN_PASS_SCORE: 20 preguntas al azar de 50, apruebas con 80')
...
values
  ('pbn_evaluacion', 'PBN · Evaluación', 'al_final', 20, 80,
   true, 180, 'pbn', true, 'pbn')
...
-- De un solo banco y sin cupo: el intento se sortea entero sobre las 50.
insert into public.evaluacion_fuentes (evaluacion, banco, etiqueta, cupo)
values ('pbn_evaluacion', 'pbn_evaluacion', null, null)
on conflict (evaluacion, banco) do update set
  etiqueta = excluded.etiqueta,
  cupo = excluded.cupo;

-- ── Los cuatro logros ──
-- 46 a 49: los anteriores llegan hasta el 45 (RVSM).
insert into public.achievements (code, name, description, icon, tier, order_index) values
  ('pbn_lesson', 'PBN leído', 'Leíste los cincuenta y dos capítulos del módulo PBN', '📖', 'bronze', 46),
  ('pbn_practice', 'PBN practicado', 'Respondiste las ciento cincuenta y seis preguntas de práctica del módulo PBN', '📐', 'silver', 47),
  ('pbn_exam', 'Evaluación de PBN superada', 'Aprobaste la evaluación del módulo PBN', '✅', 'silver', 48),
  ('pbn_master', 'PBN dominado', 'Terminaste el módulo entero: lección, práctica y evaluación', '🧭', 'gold', 49)
  ...
  if v_s.terminada_en is null then
    ...
    case v_eval.destino
      ...
      when 'pbn' then
        ...
        values (v_sesion.user_id, v_puntaje, v_correctas, v_total);
      when 'rvsm' then
...
drop trigger if exists trg_check_achievements_rvsm on public.user_rvsm_progress;
create trigger trg_check_achievements_rvsm
  after insert or update on public.user_rvsm_progress
  for each row execute function private.trigger_logros('rvsm');

drop trigger if exists trg_check_achievements_rvsm_exam on public.user_rvsm_exam_attempts;
create trigger trg_check_achievements_rvsm_exam
  after insert on public.user_rvsm_exam_attempts
  for each row execute function private.trigger_logros('rvsm');
...
begin
  ...
  foreach v_grupo in array array[
    ...
    'notam', 'metar', 'aerolinea', 'mercancias', 'aerodinamica', 'aeropuertos', 'performance', 'comunicaciones', 'rac', 'combustible', 'rvsm', 'pbn'
  ] loop
