-- ============================================================================
-- Módulo MEL: panel y logros.
--
-- NO SE HA APLICADO. Se le entrega a Camilo para que la corra él, y después
-- se registra con la versión del nombre de archivo
-- (`supabase migration repair --status applied 20261001020000`, ver
-- docs/MEL_ESTADO.md).
--
-- Es el recorrido de 20260927020000_panel_y_logros_de_comunicaciones.sql:
--
--   1. (Las claves de práctica NO: van por scripts/catalogo, ver la sección 1.)
--   2. private.practicas_hechas con la rama de mel.
...
-- más reciente, que es la de 20260929120000_modulo_pbn (aplicada en
-- producción, con los once módulos), y lo que cambia en cada una es la rama
-- de 'mel'. panel_tarjetas conserva 'plan', 'postulaciones', 'licencias',
-- 'preparacion' y los once módulos de antes.
-- scripts/migraciones/funciones-compartidas.test.ts lo comprueba.
-- 20260930000000 no toca ninguna de estas.
--
-- Un arreglo sobre la copia: el archivo de PBN deja una coma de más después
-- de la tarjeta de rvsm en panel_tarjetas (`),,`), que no compila. Aquí va
-- una sola.
--
-- ORDEN: DESPUÉS de 20261001010000_evaluacion_de_mel.sql, porque el panel y
-- los logros leen user_mel_exam_attempts, que nace allí, y comparan contra el
-- umbral mel_pass. Y ninguna migración anterior se vuelve a correr después
-- de esta: la que corre última decide qué módulos conocen estas funciones.
--
-- Se puede correr dos veces: todo es `on conflict`, `drop … if exists` o
-- `create or replace`.
--
-- Prueba: supabase/tests/mel.sql (progreso, puerta, panel y logros) y el caso
-- de MEL en supabase/tests/logros.sql.
-- ============================================================================

-- ── 1 · Catálogo: la práctica ──────────────────────────────────────────────
--
-- Esta migración NO escribe las claves de práctica, como la de
-- Comunicaciones: una lista copiada aquí quedaría vieja en cuanto cambie un
-- ejercicio. Las claves las carga el paso que sigue a esta migración
-- (docs/MEL_ESTADO.md), con la salida de
--   node scripts/catalogo/sembrar.mjs mel
-- que sale de contenido/catalogo/modulos.json (claveEjercicioMel sobre los
-- ejercicios; scripts/catalogo/catalogo.test.ts falla si no está al día).
--
-- Mientras tanto la fila sigue con 40 lecciones y ninguna práctica, como la
-- dejó 20261001000000: la RPC rechaza toda clave y el logro de práctica no se
-- puede ganar (ver la guarda en desbloquear_logros). Nada se rompe.

-- ── 2 · Contar lo practicado ───────────────────────────────────────────────
--
-- La de 20260929120000 (los once módulos) con una rama más. (secciones_leidas
-- ya la ganó en 20261001010000, para la puerta de la evaluación.)
...
as $function$
  ...
  from (
    ...
    union all
    select unnest(practice_done) from public.user_rvsm_progress where p_modulo = 'rvsm' and user_id = p_user
    union all
    select unnest(practice_done) from public.user_pbn_progress where p_modulo = 'pbn' and user_id = p_user
    union all
    select unnest(practice_done) from public.user_mel_progress where p_modulo = 'mel' and user_id = p_user
  ) as hechas
  where exists (
...
-- Orden 50 a 53: después de los de PBN (46 a 49). RAC usa 34 a 37,
-- Combustible 38 a 41 y RVSM 42 a 45.
insert into public.achievements (code, name, description, icon, tier, order_index) values
  ('mel_lesson', 'MEL leída', 'Leíste las cuarenta lecciones del módulo de MEL', '📖', 'bronze', 50),
  ('mel_practice', 'MEL practicada', 'Resolviste todos los ejercicios de la práctica con entradas de MEL', '📋', 'silver', 51),
  ('mel_exam', 'Evaluación de MEL superada', 'Aprobaste la evaluación de MEL', '✅', 'silver', 52),
  ('mel_master', 'MEL dominada', 'Terminaste el módulo entero: lección, práctica y evaluación', '🛠️', 'gold', 53)
...
--
-- La de 20260929120000 (los once módulos) con el grupo 'mel' agregado. La
-- práctica exige además que el catálogo tenga alguna: con cero claves,
-- «todas» se cumpliría sin hacer nada (es el caso entre esta migración y la
-- siembra del catálogo, y el de 20261001000000 corrida otra vez).
...
begin
  ...
  v_codigos := case p_grupo
    ...
    when 'rvsm' then array['rvsm_lesson', 'rvsm_practice', 'rvsm_exam', 'rvsm_master']
    when 'pbn' then array['pbn_lesson', 'pbn_practice', 'pbn_exam', 'pbn_master']
    when 'mel' then array['mel_lesson', 'mel_practice', 'mel_exam', 'mel_master']
  ...
  when 'rac' then
    ...
    -- Con cero claves en el catálogo, «todas» se cumpliría sin hacer nada.
  ...
  when 'rvsm' then
    v_lecciones := private.secciones_leidas(p_user, 'rvsm')
      >= (select c.lecciones from public.modulos_contenido c where c.modulo = 'rvsm');
    v_practicas := (select cardinality(c.practicas) from public.modulos_contenido c where c.modulo = 'rvsm') > 0
      and private.practicas_hechas(p_user, 'rvsm')
        >= (select cardinality(c.practicas) from public.modulos_contenido c where c.modulo = 'rvsm');
    v_aprobado := exists (
      select 1 from public.user_rvsm_exam_attempts
      where user_id = p_user
        and coalesce(score, 0) >= coalesce((select total from public.module_thresholds where code = 'rvsm_pass'), 80)
    );
    if v_lecciones then v_new := v_new + public._try_unlock(p_user, 'rvsm_lesson'); end if;
    if v_practicas then v_new := v_new + public._try_unlock(p_user, 'rvsm_practice'); end if;
    if v_aprobado then v_new := v_new + public._try_unlock(p_user, 'rvsm_exam'); end if;
    if v_lecciones and v_practicas and v_aprobado then
      v_new := v_new + public._try_unlock(p_user, 'rvsm_master');
    end if;

  when 'pbn' then
    v_lecciones := private.secciones_leidas(p_user, 'pbn')
      >= (select c.lecciones from public.modulos_contenido c where c.modulo = 'pbn');
    v_practicas := (select cardinality(c.practicas) from public.modulos_contenido c where c.modulo = 'pbn') > 0
      and private.practicas_hechas(p_user, 'pbn')
        >= (select cardinality(c.practicas) from public.modulos_contenido c where c.modulo = 'pbn');
    v_aprobado := exists (
      select 1 from public.user_pbn_exam_attempts
      where user_id = p_user
        and coalesce(score, 0) >= coalesce((select total from public.module_thresholds where code = 'pbn_pass'), 80)
    );
    if v_lecciones then v_new := v_new + public._try_unlock(p_user, 'pbn_lesson'); end if;
    if v_practicas then v_new := v_new + public._try_unlock(p_user, 'pbn_practice'); end if;
    if v_aprobado then v_new := v_new + public._try_unlock(p_user, 'pbn_exam'); end if;
    if v_lecciones and v_practicas and v_aprobado then
      v_new := v_new + public._try_unlock(p_user, 'pbn_master');
    end if;

  when 'mel' then
    v_lecciones := private.secciones_leidas(p_user, 'mel')
      >= (select c.lecciones from public.modulos_contenido c where c.modulo = 'mel');
    -- Sin práctica en el catálogo no hay logro de práctica: cardinality de un
    -- arreglo vacío es 0 y cualquiera lo cumpliría.
    v_practicas := (select cardinality(c.practicas) from public.modulos_contenido c where c.modulo = 'mel') > 0
      and private.practicas_hechas(p_user, 'mel')
        >= (select cardinality(c.practicas) from public.modulos_contenido c where c.modulo = 'mel');
    v_aprobado := exists (
      select 1 from public.user_mel_exam_attempts
      where user_id = p_user
        and coalesce(score, 0) >= coalesce((select total from public.module_thresholds where code = 'mel_pass'), 80)
    );
    if v_lecciones then v_new := v_new + public._try_unlock(p_user, 'mel_lesson'); end if;
    if v_practicas then v_new := v_new + public._try_unlock(p_user, 'mel_practice'); end if;
    if v_aprobado then v_new := v_new + public._try_unlock(p_user, 'mel_exam'); end if;
    if v_lecciones and v_practicas and v_aprobado then
      v_new := v_new + public._try_unlock(p_user, 'mel_master');
    end if;
...
drop trigger if exists trg_check_achievements_mel on public.user_mel_progress;
create trigger trg_check_achievements_mel
  after insert or update on public.user_mel_progress
  for each row execute function private.trigger_logros('mel');

drop trigger if exists trg_check_achievements_mel_exam on public.user_mel_exam_attempts;
create trigger trg_check_achievements_mel_exam
  after insert or update on public.user_mel_exam_attempts
  for each row execute function private.trigger_logros('mel');
...
begin
  ...
  foreach v_grupo in array array[
    ...
    'notam', 'metar', 'aerolinea', 'mercancias', 'aerodinamica', 'aeropuertos', 'performance', 'comunicaciones', 'rac', 'combustible', 'rvsm', 'pbn', 'mel'
  ] loop
...
--
-- La de 20260929120000 (plan, postulaciones, licencias, preparación y los
-- once módulos) con la tarjeta de 'mel'. No se quita nada.
...
begin
  ...
  return jsonb_build_object(
    ...
    'rvsm', jsonb_build_object(
      'lecciones', private.secciones_leidas(v_user, 'rvsm'),
      'practicas', private.practicas_hechas(v_user, 'rvsm'),
      'mejor', (select max(e.score) from public.user_rvsm_exam_attempts e where e.user_id = v_user)
    ),
    'pbn', jsonb_build_object(
      'lecciones', private.secciones_leidas(v_user, 'pbn'),
      'practicas', private.practicas_hechas(v_user, 'pbn'),
      'mejor', (select max(e.score) from public.user_pbn_exam_attempts e where e.user_id = v_user)
    ),
    'mel', jsonb_build_object(
      'lecciones', private.secciones_leidas(v_user, 'mel'),
      'practicas', private.practicas_hechas(v_user, 'mel'),
      'mejor', (select max(e.score) from public.user_mel_exam_attempts e where e.user_id = v_user)
    ),
